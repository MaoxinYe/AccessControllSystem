...
<article class="page-container">
	<form action="system-gates-edit" method="post" class="form form-horizontal" id="form-gates-manage-edit">
		<div class="tabBar cl">
			<span>修改门禁闸机信息</span>
		</div>
		<input type="text" class="input-text" value="${gates.gatesid}" hidden="hidden" id="gatesid" name="gatesid">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<select class="select" size="1" id="areacode" name="areacode" style="width: 200px;height: 31px" onchange="myChange(this.id)">
					...
					<c:forEach var="area" items="${areaList}">
						<option value="${area.areacode}" <c:if test="${gates.areacode==area.areacode}">selected="selected"</c:if>>${area.areaname}</option>
					</c:forEach>
				</select>
			</div>
		</div>
			...
			<div class="formControls col-xs-8 col-sm-9">
				<select class="select" size="1" id="areacode2" name="areacode2" style="width: 200px;height: 31px">
					...
					<c:forEach var="area" items="${departmentList}">
						<c:if test="${area.supercode eq gates.areacode }">
							<option value="${area.areacode}" <c:if test="${gates.areacode2==area.areacode}">selected="selected"</c:if>> ${area.areaname}</option>
						</c:if>
					</c:forEach>
				</select>
			</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${gates.gatesname}" placeholder="请输入硬件说明信息" id="gatesname" name="gatesname" style="width:200px">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<select class="select" size="1" id="type" name="type" style="width: 200px;height: 31px">
					<option value="">--请选择--</option>
					<option value="1" <c:if test="${gates.type==1}">selected="selected"</c:if>>门禁</option>
					<option value="2" <c:if test="${gates.type==2}">selected="selected"</c:if>>闸机</option>
				</select>
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<select class="select" size="1" id="inorout" name="inorout" style="width: 200px;height: 31px">
					<option value="">--请选择--</option>
					<option value="1" <c:if test="${gates.inorout==1}">selected="selected"</c:if>>进</option>
					<option value="2" <c:if test="${gates.inorout==2}">selected="selected"</c:if>>出</option>
				</select>
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${gates.address}" placeholder="" id="address" name="address" style="width:200px">
			</div>
		</div>
		<div class="row cl">
...
$(function(){
	$("#form-gates-manage-edit").validate({
		debug:true,
		rules:{
			...
			gatesname:{ required:true, maxLengthInCN: 50,
				remote:{
                    ...
                    data: {
                    	...
                    	gatesname: function () {if($("#gatesname").val() !== "${gates.gatesname}"){return $("#gatesname").val();} else { return false;} }
                     },
                },
			},
		...
		submitHandler:function(form){
			$(form).ajaxSubmit({
					...
					success: function(data){
						if(data.head){
							layer.msg("门禁闸机修改成功!",{icon:1,time:1000},function(){
								parent.query();
