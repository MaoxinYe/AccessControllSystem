...
<article class="page-container">
	<form action="system-visitor-add" method="post" class="form form-horizontal" id="form-visitor-manage-add">
		<div class="tabBar cl">
			<span>添加访客信息</span>
		</div>		
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="请输入访客姓名" id="name" name="name" style="width:200px">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" autocomplete="off" value="" placeholder="请输入证件号" id="credentials" name="credentials" style="width:200px">
		</div>
		</div>
		<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>访客电话：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" autocomplete="off"  placeholder="请输入访客电话" id="tel" name="tel" style="width:200px">
		</div>
		</div>
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" style="width:160px" placeholder="选择访问开始时间" id="startTime" name="time" value="${conditions.startTime}" onfocus="WdatePicker()" class="input-text Wdate">
         	<%-- --
        	<input type="text" style="width:160px" placeholder="选择访问结束时间" id="endTime" name="endTime" value="${conditions.endTime}" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'startTime\')}'})" class="input-text Wdate"> --%>
		</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>受访人员姓名：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="请输入受访人员姓名" id="personnel_name" name="personnel_name" style="width:200px">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>受访人员电话：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="请输入受访人员电话" id="personnel_tel" name="personnel_tel" style="width:200px">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">备注：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="请输入备注" id="remarks" name="remarks" style="width:200px">
			</div>
		</div>
		<div class="row cl" >
        ...
        <div class="formControls col-xs-8 col-sm-9">
            ...
            <img id="img1" src="" width="171px" height="210px" alt="预览"/>
            </div>
        </div>
        </div>
		<div class="row cl">
...
$(function(){
	$("#form-visitor-manage-add").validate({
		debug:true,
		rules:{
			name:{required:true,maxLengthInCN:50},
			credentials:{required:true,maxLengthInCN:30,checkENGNUM:true},
			tel:{required:true,isTel:true},
			personnel_name:{required:true,maxLengthInCN:50},
			personnel_tel:{required:true,isTel:true},
			remarks:{maxLengthInCN:50},
            file:{required:true,extension:"jpe?g"},
		},
		messages: {
			name:{required:"访客姓名不能为空",maxLengthInCN:"访客姓名长度不能大于50个字符"},
			credentials:{required:"访客证件号不能为空",maxLengthInCN:"证件号长度不能大于30个字符",checkENGNUM:"证件号只能是字母和数字"},
			...
			remarks:{maxLengthInCN:"备注长度不能大于50个字符"},
            file:{required:"访客标准照不能为空",extension:"只能上传.jpg格式的照片"},
		},
		onkeyup:false,
		focusCleanup:false,
		success:"valid",
		submitHandler:function(form){
			$(form).ajaxSubmit({
					...
					success: function(data){
						if(data.head){
							layer.msg("访客添加成功!",{icon:1,time:1000},function(){
								parent.query();
