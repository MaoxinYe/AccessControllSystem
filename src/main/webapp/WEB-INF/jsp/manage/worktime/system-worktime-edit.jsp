...
<article class="page-container" style="padding-left: 200px">
	<form action="system-worktime-edit" method="post" class="form form-horizontal" id="form-system-worktime-edit">
		<input type="text" hidden="hidden" class="input-text" value="${worktime.worktimeid}" placeholder="" id="id" name="id" style="width:200px">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>上班时间：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" style="width:160px" value="${startTime}"  placeholder="选择上班时间" id="startTime" name="startTime" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'endTime\')}',dateFmt:'HH:mm'})" class="input-text Wdate">
			</div>
		</div>	
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>下班时间：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" style="width:160px" value="${endTime}"  placeholder="选择下班时间" id="endTime" name="endTime" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'startTime\')}',dateFmt:'HH:mm'})" class="input-text Wdate">
			</div>
		</div>	
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>班次名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${worktime.illustrate}" placeholder="" id="illustrate" name="illustrate" style="width:200px">
			</div>
		</div>		
		<div class="row cl">
...
$(function(){	
	$("#form-system-worktime-edit").validate({
		debug:true,
		rules:{
			illustrate:{ required:true,maxLengthInCN:200,
				remote:{
                    depends : function(element) {
                        return element.value !== "${worktime.illustrate}";
                    },
                    param : {
                        url : "check-illustrate.html",
                        cache :false
                    },
                },
			},
		...
		submitHandler:function(form){
			$(form).ajaxSubmit({
					...
					success: function(data){
						if(data.head){
							layer.msg("作息时间修改成功!",{icon:1,time:1000},function(){
